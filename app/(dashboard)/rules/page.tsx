...
export default function RulesPage() {
  ...
  useEffect(() => {
    ...
    if (q) out = out.filter(r => r.title.toLowerCase().includes(q)||(r.mitre_id||'').toLowerCase().includes(q)||(r.tactic||'').toLowerCase().includes(q)||(r.scenario||'').toLowerCase().includes(q))
  ...
  async function load() {
    setLoading(true)
    const { data } = await createClient().from('rules').select('*').order('created_at', { ascending: false })
  ...
  async function shareRule(rule: DbRule) {
    ...
    if (newPublic) { const url = `${window.location.origin}/share/${rule.id}`; setShareUrl(url); navigator.clipboard.writeText(url) }
    setSharing(false); load()
  }
  ...
  async function scoreRule(rule: DbRule) {
    ...
    setQuality(data.quality); setScoring(false)
  }
  ...
  async function bulkDelete() {
    ...
    if (!confirm(`Permanently delete ${selected.size} rules? This cannot be undone.`)) return
    setBulkDeleting(true)
    const ids = Array.from(selected)
    await createClient().from('rules').delete().in('id', ids)
    setRules(p => p.filter(r => !selected.has(r.id)))
    if (sel && selected.has(sel.id)) setSel(null)
    setSelected(new Set())
  ...
  function dlKQL(r: DbRule) {
    ...
    const a = document.createElement('a'); a.href = URL.createObjectURL(new Blob([txt],{type:'text/plain'})); a.download = (r.mitre_id||'rule').replace(/\./g,'_')+'_detection.kql'; a.click()
  }
  function dlARM(r: DbRule) {
    ...
    const a = document.createElement('a'); a.href = URL.createObjectURL(new Blob([JSON.stringify(arm,null,2)],{type:'application/json'})); a.download = (r.mitre_id||'rule').replace(/\./g,'_')+'_arm.json'; a.click()
  }
  ...
  function dlSigma(r: DbRule) {
    const a=document.createElement('a'); a.href=URL.createObjectURL(new Blob([toSigma(r)],{type:'text/plain'})); a.download=(r.mitre_id||'rule').replace(/\./g,'_')+'_sigma.yml'; a.click()
  }
  async function generatePlaybook(rule: DbRule) {
    setPlayLoading(true); setShowPlay(true); setPlaybook(null)
    try { const res=await fetch('/api/playbook',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({rule})}); const data=await res.json(); if(!res.ok)throw new Error(data.error); setPlaybook(data.playbook) }
    catch(e:unknown){console.error(e)} finally{setPlayLoading(false)}
  }
  async function testRule(rule: DbRule) {
    if(!testLog.trim())return; setTestLoading(true); setTestResult(null)
    try {
      ...
      const a=data.analysis; const wouldMatch=a.threat_level!=='Benign'&&a.mitre_techniques?.some((t:any)=>t.id===rule.mitre_id)
      setTestResult(wouldMatch?`✅ Rule would likely FIRE — ${a.threat_level}. MITRE ${rule.mitre_id} matched.`:`⚠️ Rule may NOT fire — ${a.threat_level==='Benign'?'log appears benign':'MITRE mismatch'}. Detected: ${a.mitre_techniques?.map((t:any)=>t.id).join(', ')||'none'}`)
    } catch(e:unknown){setTestResult('Test failed: '+(e instanceof Error?e.message:'unknown'))} finally{setTestLoading(false)}
  }
  async function validateRule(rule: DbRule) {
    try { const res=await fetch('/api/validate',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({rule:rule.rule,platform:rule.platform})}); const data=await res.json(); setValidation(data) }
    catch{/*silent*/}
  }
  ...
  return (
    <div style={{display:'grid', gridTemplateColumns:'340px 1fr', height:'calc(100vh - 48px)', overflow:'hidden'}}>

      {/* ── LEFT: Rule list (independently scrollable) ─────────────────── */}
      <div style={{background:'var(--bg4)',borderRight:'1px solid var(--border)',display:'flex',flexDirection:'column',overflow:'hidden'}}>
        ...
        <div style={{padding:'.85rem 1rem',borderBottom:'1px solid var(--border)',display:'flex',flexDirection:'column',gap:'.45rem',flexShrink:0}}>
          ...
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between'}}>
            <div style={{fontSize:'.63rem',color:'var(--muted2)',fontFamily:'var(--font-mono)'}}>{filtered.length} of {rules.length} rules</div>
            {filtered.length > 0 && (
              <label style={{display:'flex',alignItems:'center',gap:'.35rem',fontSize:'.68rem',color:'var(--muted)',cursor:'pointer'}}>
                <input type="checkbox" checked={selected.size>0&&selected.size===filtered.length} onChange={selectAll}
        ...
        {/* Bulk action bar */}
        {selected.size > 0 && (
          <div style={{padding:'.55rem 1rem',background:'rgba(239,68,68,.08)',borderBottom:'1px solid rgba(239,68,68,.2)',display:'flex',alignItems:'center',justifyContent:'space-between',flexShrink:0}}>
            <span style={{fontSize:'.78rem',color:'var(--red)',fontWeight:600}}>{selected.size} rule{selected.size>1?'s':''} selected</span>
            <div style={{display:'flex',gap:'.5rem'}}>
              <button onClick={()=>setSelected(new Set())} style={{...btn,fontSize:'.72rem'}}>Clear</button>
              <button onClick={bulkDelete} disabled={bulkDeleting}
                style={{padding:'.3rem .8rem',borderRadius:6,border:'none',background:'var(--red)',color:'#fff',fontSize:'.72rem',cursor:bulkDeleting?'not-allowed':'pointer',fontFamily:'inherit',fontWeight:600}}>
          ...
          {filtered.map(rule => {
            ...
            return (
              <div key={rule.id} onClick={()=>{setSel(rule);setTab('desc')}}
                style={{padding:'.72rem 1rem',borderBottom:'1px solid var(--border)',cursor:'pointer',
                  background:isSelected?'rgba(239,68,68,.06)':active?'rgba(249,115,22,.07)':'transparent',
                  borderLeft:`3px solid ${isSelected?'var(--red)':active?'#f97316':'transparent'}`,transition:'all .12s'}}>
                <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',gap:'.5rem'}}>
                  {/* Checkbox */}
                  <div onClick={e=>toggleSelect(rule.id,e)} style={{paddingTop:'.1rem',flexShrink:0}}>
                  ...
                  <button onClick={e=>{e.stopPropagation();toggleFav(rule)}} style={{background:'none',border:'none',cursor:'pointer',fontSize:'.82rem',opacity:rule.is_favourite?1:.25,transition:'opacity .15s',padding:'.1rem'}}>⭐</button>
                </div>
              </div>
            )
          })}
      ...
      {/* ── RIGHT: Rule detail (independently scrollable) ─────────────── */}
      <div style={{background:'var(--bg)',overflowY:'auto'}}>
        {!sel ? (
          <div style={{display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',minHeight:'100%',gap:'.75rem',textAlign:'center'}}>
            ...
            {rules.length > 0 && (
              <div style={{fontSize:'.75rem',color:'var(--muted2)',marginTop:'.5rem'}}>
                Use checkboxes to select multiple rules for bulk deletion
              </div>
            )}
          </div>
        ) : (
          <div style={{padding:'1.25rem',maxWidth:900}}>
            <div style={{background:'var(--bg4)',border:'1px solid var(--border)',borderRadius:14,overflow:'hidden',boxShadow:'0 3px 10px rgba(0,0,0,.1)'}}>
              ...
              <div style={{padding:'.9rem 1.15rem',background:'linear-gradient(135deg,rgba(249,115,22,.05),rgba(99,102,241,.04))',borderBottom:'1px solid var(--border)',display:'flex',alignItems:'flex-start',justifyContent:'space-between',gap:'.65rem',flexWrap:'wrap'}}>
                <div style={{flex:1,minWidth:0}}>
                  ...
                  <div style={{display:'flex',flexWrap:'wrap',gap:'.3rem'}}>
                    {[
                      sel.severity&&{label:sel.severity,c:SEV[sel.severity]||SEV.Low},
                      sel.mitre_id&&{label:sel.mitre_id,c:{bg:'var(--blue-bg)',fg:'var(--blue)',bd:'var(--blue-bd)'}},
                      sel.tactic&&{label:sel.tactic,c:{bg:'var(--purple-bg)',fg:'var(--purple)',bd:'var(--purple-bd)'}},
                      sel.data_source&&{label:sel.data_source,c:{bg:'var(--green-bg)',fg:'var(--green)',bd:'var(--green-bd)'}},
                      sel.confidence!=null&&{label:`${sel.confidence}% confidence`,c:{bg:'var(--green-bg)',fg:'var(--green)',bd:'var(--green-bd)'}},
              ...
              <div style={{padding:'.9rem 1.1rem'}}>
                {tab==='desc'&&<p style={{fontSize:'.82rem',color:'var(--text2)',lineHeight:1.75,margin:0}}>{esc(sel.description)}{sel.mitre_name&&<><br/><br/><span style={{fontSize:'.72rem',color:'var(--muted)'}}>MITRE: <strong style={{color:'#f97316'}}>{sel.mitre_id} — {esc(sel.mitre_name)}</strong></span></>}</p>}
              ...
              {validation && (
                <div style={{padding:'.75rem 1.1rem',borderTop:'1px solid var(--border)',background:validation.valid?'rgba(52,211,153,.03)':'rgba(248,113,113,.03)'}}>
                  ...
                  {[...(validation.errors||[]).map((e:string)=>({t:'❌ Error',m:e})),...(validation.warnings||[]).map((w:string)=>({t:'⚠️ Warning',m:w})),...(validation.suggestions||[]).map((s:string)=>({t:'💡',m:s}))].map((item,i)=>(
              ...
              {showTest && (
                <div style={{padding:'.85rem 1.1rem',borderTop:'1px solid var(--border)',background:'rgba(99,102,241,.03)'}}>
                  <div style={{fontSize:'.62rem',fontWeight:700,textTransform:'uppercase',letterSpacing:'.09em',color:'var(--muted)',marginBottom:'.5rem'}}>Test rule against a log</div>
                  <textarea value={testLog} onChange={e=>setTestLog(e.target.value)} rows={4} placeholder="Paste a sample log entry here..."
                  ...
                  <div style={{display:'flex',gap:'.5rem',alignItems:'center'}}>
                    ...
                    {testResult&&<span style={{fontSize:'.78rem',color:'var(--muted)',flex:1}}>{testResult}</span>}
                  </div>
                </div>
              )}
              ...
              {showPlay && (
                <div style={{padding:'.85rem 1.1rem',borderTop:'1px solid var(--border)',background:'rgba(249,115,22,.03)'}}>
                  ...
                  {playLoading?<div style={{color:'var(--muted)',fontSize:'.8rem'}}>Generating playbook...</div>:playbook&&(
                    <div style={{display:'flex',flexDirection:'column',gap:'.65rem'}}>
                      {playbook.overview&&<p style={{fontSize:'.8rem',color:'var(--text2)',lineHeight:1.7,margin:0}}>{playbook.overview}</p>}
                      {[['🔍 Triage','triage'],['🔬 Investigation','investigation'],['🛡 Containment','containment'],['🧹 Eradication','eradication'],['📦 Evidence','evidence_to_collect'],['📢 Escalation','escalation_criteria']].map(([label,key])=>playbook[key]?.length&&(
                        <div key={key}>
              ...
              {shareUrl&&(
                <div style={{margin:'0 1rem .5rem',padding:'.65rem .9rem',background:'var(--green-bg)',border:'1px solid var(--green-bd)',borderRadius:8,display:'flex',alignItems:'center',gap:'.75rem',flexWrap:'wrap'}}>
                  <span style={{fontSize:'.75rem',color:'var(--green)',fontWeight:600}}>Rule shared — link copied</span>
              ...
              {quality&&(
                <div style={{margin:'0 1rem .5rem',padding:'.85rem 1rem',background:'var(--bg4)',border:'1px solid var(--border)',borderRadius:10}}>
                  ...
                  <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(140px,1fr))',gap:'.4rem',marginBottom:'.6rem'}}>
                    {[{label:'Specificity',val:quality.specificity as number},{label:'FP risk',val:quality.fp_risk as number,invert:true},{label:'Coverage',val:quality.coverage as number},{label:'Implementation',val:quality.implementation as number}].map(m=>(
                  ...
                  {(quality.improvements as string[])?.length>0&&<div style={{fontSize:'.72rem',color:'var(--muted2)'}}>Improvements: {(quality.improvements as string[]).join(' · ')}</div>}
                </div>
              )}
              ...
              <div style={{padding:'.75rem 1.1rem',background:'rgba(249,115,22,.03)',borderTop:'1px solid var(--border)'}}>
                ...
                {editNote===sel.id?(
                  ...
                  <div style={{display:'flex',alignItems:'center',gap:'.75rem'}}>
                    <span style={{fontSize:'.78rem',color:sel.note?'var(--text2)':'var(--muted2)',fontStyle:sel.note?'normal':'italic',flex:1}}>{sel.note||'No note yet'}</span>
                    <button onClick={()=>{setEditNote(sel.id);setNoteVal(sel.note||'')}} style={{padding:'.2rem .55rem',borderRadius:5,border:'1px solid var(--border)',background:'var(--bg)',color:'var(--muted)',fontSize:'.68rem',cursor:'pointer',fontFamily:'inherit'}}>{sel.note?'Edit':'+ Add'}</button>
                  </div>
                )}
              </div>
              ...
              <div style={{padding:'.75rem 1.1rem',background:'rgba(34,197,94,.03)',borderTop:'1px solid var(--border)',display:'flex',alignItems:'center',justifyContent:'space-between',gap:'1rem',flexWrap:'wrap'}}>
                <div>
                  ...
                  <div style={{fontSize:'.72rem',color:'var(--muted2)'}}>{(sel as any).last_reviewed_at?`Last reviewed ${new Date((sel as any).last_reviewed_at).toLocaleDateString()}`:'Never reviewed'}</div>
                </div>
                <button onClick={()=>markReviewed(sel)} disabled={reviewing===sel.id}
                  style={{padding:'.4rem .9rem',borderRadius:7,border:'1px solid var(--green-bd)',background:'var(--green-bg)',color:'var(--green)',fontSize:'.75rem',cursor:'pointer',fontFamily:'inherit'}}>
