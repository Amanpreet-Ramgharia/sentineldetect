...
export default function SocPage() {
  ...
  const [selPb,setSelPb]=useState<Playbook|null>(null);const [genMitre,setGenMitre]=useState('')
  const [genTactic,setGenTactic]=useState('');const [genCtx,setGenCtx]=useState('');const [generating,setGenerating]=useState(false)
  ...
  async function submitAlert(){
    ...
    setAlertText('');setShowSubmit(false);setAlerts(p=>[d.alert,...p]);setTotalAlerts(t=>t+1);setExpanded(d.alert.id);setSubmitting(false)
  }
  ...
  const TAB_COUNTS: Record<Tab,number> = {alerts:totalAlerts,cases:totalCases,playbooks:playbooks.length,analytics:0}
    ...
    <div style={{minHeight:'100vh',background:'var(--bg)',color:'var(--text)',padding:'1.5rem 2rem'}}>
      ...
      <div style={{display:'flex',gap:'.35rem',marginBottom:'1.25rem',borderBottom:'1px solid var(--border)',paddingBottom:'.75rem'}}>
        {(['alerts','cases','playbooks','analytics'] as Tab[]).map(t=>(
          <button key={t} onClick={()=>setTab(t)} style={tabBtn(t)}>
            {t.charAt(0).toUpperCase()+t.slice(1)}{t!=='analytics'&&TAB_COUNTS[t]>0?` (${TAB_COUNTS[t]})`:''
            }
          </button>
        ))}
      </div>
        ...
        <>
          {showSubmit&&(
            <div style={{background:'var(--bg4)',border:'1px solid var(--border)',borderRadius:12,padding:'1.25rem',marginBottom:'1.25rem'}}>
              ...
              <div style={{display:'flex',gap:'.65rem'}}>
                <button onClick={submitAlert} disabled={submitting||!alertText.trim()} style={{padding:'.55rem 1.25rem',borderRadius:8,background:submitting?'var(--muted2)':'#f97316',border:'none',color:'#fff',fontWeight:700,fontSize:'.85rem',cursor:submitting?'not-allowed':'pointer',fontFamily:'inherit'}}>{submitting?'Triaging…':'Triage with AI'}</button>
          ...
          {alerts.map(alert=>(
            <div key={alert.id} style={{background:'var(--bg4)',border:`1px solid ${alert.escalate?'rgba(239,68,68,.4)':'var(--border)'}`,borderRadius:12,marginBottom:'.6rem',overflow:'hidden'}}>
              <div style={{display:'flex',alignItems:'center',gap:'.75rem',padding:'1rem 1.2rem',cursor:'pointer'}} onClick={()=>setExpanded(expanded===alert.id?null:alert.id)}>
                ...
                <div style={{flex:1,minWidth:0}}>
                  <div style={{fontWeight:600,fontSize:'.88rem',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{alert.escalate&&<span style={{color:'var(--red)',marginRight:'.35rem'}}>⚑</span>}{alert.title}</div>
              ...
              {expanded===alert.id&&(
                <div style={{borderTop:'1px solid var(--border)',padding:'1.2rem'}}>
                  <div style={{marginBottom:'1rem'}}><div style={{fontSize:'.72rem',fontWeight:600,color:'var(--muted)',textTransform:'uppercase',letterSpacing:'.08em',marginBottom:'.5rem'}}>Summary</div><div style={{fontSize:'.85rem',color:'var(--muted)',lineHeight:1.65}}>{alert.summary}</div></div>
      ...
      {tab==='cases'&&(
        <>
          ...
          {!casesLoading&&cases.length===0&&<div style={{color:'var(--muted)',textAlign:'center',padding:'3rem',fontSize:'.85rem',background:'var(--bg4)',border:'1px solid var(--border)',borderRadius:12}}>No cases yet. Open an alert and click "Create case →" to start an investigation.</div>}
          {cases.map(c=>(
            <div key={c.id} style={{background:'var(--bg4)',border:'1px solid var(--border)',borderRadius:12,marginBottom:'.6rem',padding:'1rem 1.2rem',cursor:'pointer',display:'flex',alignItems:'center',gap:'.75rem'}} onClick={()=>router.push(`/soc/cases/${c.id}`)}>
              ...
              <div style={{flex:1,minWidth:0}}>
                ...
                <div style={{fontSize:'.75rem',color:'var(--muted)',marginTop:'.2rem',display:'flex',gap:'.75rem',flexWrap:'wrap'}}>
                  ...
                  <span>Open {dur(c.opened_at)}</span>
                  <span>{tAgo(c.created_at)}</span>
      ...
      {tab==='playbooks'&&(
        <>
          {showGenForm&&(
            <div style={{background:'var(--bg4)',border:'1px solid var(--border)',borderRadius:12,padding:'1.25rem',marginBottom:'1.25rem'}}>
              ...
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'.65rem',marginBottom:'.65rem'}}>
                <input value={genMitre} onChange={e=>setGenMitre(e.target.value)} placeholder="MITRE technique (e.g. T1059)" style={inp}/>
                <input value={genTactic} onChange={e=>setGenTactic(e.target.value)} placeholder="Tactic (e.g. Execution)" style={inp}/>
              </div>
              <textarea value={genCtx} onChange={e=>setGenCtx(e.target.value)} placeholder="Optional context — paste alert summary or description…" rows={2} style={{...inp,resize:'vertical',marginBottom:'.65rem'}}/>
              <div style={{display:'flex',gap:'.65rem'}}>
                <button onClick={generatePlaybook} disabled={generating||!genMitre.trim()} style={{padding:'.55rem 1.25rem',borderRadius:8,background:generating?'var(--muted2)':'#f97316',border:'none',color:'#fff',fontWeight:700,fontSize:'.85rem',cursor:generating?'not-allowed':'pointer',fontFamily:'inherit'}}>{generating?'Generating…':'Generate'}</button>
          ...
          {!pbLoading&&playbooks.length===0&&<div style={{color:'var(--muted)',textAlign:'center',padding:'3rem',fontSize:'.85rem',background:'var(--bg4)',border:'1px solid var(--border)',borderRadius:12}}>No playbooks yet. Click "Generate playbook" to create one for a MITRE technique.</div>}
        ...
        <>
          ...
          {analytics&&!analyticsLoading&&(
            <>
              ...
              <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(150px,1fr))',gap:'.75rem',marginBottom:'1.5rem'}}>
                {[
                  {val:analytics.overview.total,      lbl:'Total alerts'},
                  {val:analytics.overview.resolved,   lbl:'Resolved'},
                  {val:`${analytics.overview.fpRate}%`,lbl:'False positive rate'},
                  {val:analytics.overview.avgMttd===0?'—':`${analytics.overview.avgMttd}m`,lbl:'Avg MTTD'},
                  {val:analytics.overview.avgMttr===0?'—':`${analytics.overview.avgMttr}m`,lbl:'Avg MTTR (cases)'},
                ].map(({val,lbl})=>(
                  <div key={lbl} style={{background:'var(--bg4)',border:'1px solid var(--border)',borderRadius:12,padding:'1rem 1.2rem'}}>
                    <div style={{fontSize:'1.75rem',fontWeight:700,color:'#f97316'}}>{val}</div>
                    <div style={{fontSize:'.75rem',color:'var(--muted)',marginTop:'.2rem'}}>{lbl}</div>
                  ...
                  <ResponsiveContainer width="100%" height={160}>
                    <BarChart data={analytics.byMitre} layout="vertical" margin={{top:0,right:0,left:10,bottom:0}}>
                      ...
                      <Bar dataKey="count" radius={[0,4,4,0]}>
                        {analytics.byMitre.map((_,i)=><Cell key={i} fill={CHART_COLORS[i%CHART_COLORS.length]}/>)}
                      </Bar>
                    </BarChart>
                  </ResponsiveContainer>
                </div>
              ...
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'1rem'}}>
                ...
                <div style={{background:'var(--bg4)',border:'1px solid var(--border)',borderRadius:12,padding:'1.25rem'}}>
                  ...
                  {analytics.bySeverity.length===0&&<div style={{color:'var(--muted2)',fontSize:'.82rem',textAlign:'center',padding:'2rem 0'}}>No data yet</div>}
                  {analytics.bySeverity.map(({severity,count})=>{
                    ...
                    return(
                      <div key={severity} style={{marginBottom:'.65rem'}}>
                        <div style={{display:'flex',justifyContent:'space-between',marginBottom:'.25rem'}}>
                          <span style={{fontSize:'.8rem',color:SC[severity]||'var(--muted)',fontWeight:500}}>{severity}</span>
                          <span style={{fontSize:'.78rem',color:'var(--muted2)'}}>{count} ({pct}%)</span>
                        </div>
                        <div style={{background:'var(--bg)',borderRadius:999,height:6,overflow:'hidden'}}>
                          <div style={{width:`${pct}%`,height:'100%',background:SC[severity]||'#f97316',borderRadius:999,transition:'width .5s'}}/>
                        </div>
                      </div>
                    )
                  })}
                ...
                <div style={{background:'var(--bg4)',border:'1px solid var(--border)',borderRadius:12,padding:'1.25rem'}}>
                  ...
                  {analytics.bySource.length===0&&<div style={{color:'var(--muted2)',fontSize:'.82rem',textAlign:'center',padding:'2rem 0'}}>No data yet</div>}
                  {analytics.bySource.map(({source,count},i)=>{
                    const pct = analytics.overview.total>0?Math.round(count/analytics.overview.total*100):0
                    return(
                      <div key={source} style={{marginBottom:'.65rem'}}>
                        <div style={{display:'flex',justifyContent:'space-between',marginBottom:'.25rem'}}>
                          <span style={{fontSize:'.8rem',color:'var(--muted)',fontWeight:500}}>{source}</span>
                          <span style={{fontSize:'.78rem',color:'var(--muted2)'}}>{count} ({pct}%)</span>
                        </div>
                        <div style={{background:'var(--bg)',borderRadius:999,height:6,overflow:'hidden'}}>
                          <div style={{width:`${pct}%`,height:'100%',background:CHART_COLORS[i%CHART_COLORS.length],borderRadius:999,transition:'width .5s'}}/>
                        </div>
                      </div>
                    )
                  })}
