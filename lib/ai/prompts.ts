...
const PLATFORM_CONTEXT: Record<string, string> = {
  ...
  'Palo Alto Cortex XSIAM (XQL)': 'Palo Alto Cortex XSIAM using XQL (Cortex Query Language). Dataset: xdr_data. Key fields: actor_process_image_name, actor_process_command_line, action_file_path, action_network_connection_id, dst_ip_address, actor_primary_username, event_type, event_sub_type. Use filter, fields, dedup, sort, limit. Time filter: filter timestamp > to_epoch(now()-3600,"seconds").',
  'CrowdStrike Falcon (SPL)': 'CrowdStrike Falcon using Splunk-based Event Search (SPL). Search in falcon:events index. Key fields: event_simpleName (process lifecycle events), TargetProcessId, ParentProcessId, ImageFileName, CommandLine, UserName, ComputerName, aid (agent ID). Common event_simpleNames: ProcessRollup2 (process create), NetworkConnectIP4, DnsRequest, UserLogon. Use | where, | stats, | eval. Custom IOA rules use pattern matching on process trees.',
